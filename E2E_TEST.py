...
def configure_logging():
    ...
    logger.add('logs/e2e.log', level="DEBUG")
...
if __name__ == '__main__':
    ...
    track_list = p.listify_track_data(tracks, 'TEST_SERVER', '/mnt/hdd/')
    p.export_track_data(track_list, 'output/e2e_test_track_data.csv', 'TEST_SERVER')
    ...
    # get bpm data
    ids_locations = dbf.get_id_location(database)
    dbf.export_results(ids_locations, 'output/e2e_test_id_location.csv')
    dbu.process_bpm(database, 'output/e2e_test_id_location.csv')
